...
import pickle
with open("{}/splited/rare_class.pkl".format(root_path), 'wb') as f:
    pickle.dump(partial_cat, f)
with open("{}/splited/rich_class.pkl".format(root_path), 'wb') as f:
        ...
        for ea in viewpoint_list:
            if FLAG_test_sub_obj:
                tp = (os.path.join(obj_path, ea),
                      os.path.join(os.path.join(os.path.join(split_to_path_test_diff_obj, each), obj), ea))
            else:
                tp = (os.path.join(obj_path, ea),
