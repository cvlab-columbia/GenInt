...
def main_worker(gpu, ngpus_per_node, args, experiment_backup_folder):
    ...
    traindir = args.data
    import socket
    if socket.gethostname()=='cv10' or socket.gethostname()=='cv09':
        valdir = os.path.join("/local/vondrick/chengzhi/ImageNet-Data", 'val')
    elif socket.gethostname() == 'cv04':
        valdir='/local/vondrick/cz/ImageNet/val'
    elif socket.gethostname()=='thor':
        valdir = os.path.join("/local/rcs/mcz/ImageNet-Data", 'val')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, args, is_main_gpu, writer)
...
def train(train_loader, model, criterion, optimizer, epoch, args, is_main_gpu, writer):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # compute output
        images, target_a, target_b, lam = mixup_data(images, target, alpha=0.4)
        ...
        if i % 1000==1 and is_main_gpu:
            ...
            writer.add_scalar('xent loss', losses.avg, epoch*len(train_loader) + i)
