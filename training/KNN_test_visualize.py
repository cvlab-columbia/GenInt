...
def main_worker(gpu, ngpus_per_node, args):
    ...
    from learning.resnet import resnet50, resnet18, resnet152
    model=None
    ...
    LogisticRegression_test_objectnet_shuffle(test_loader, model, args, [embed_list, label_list, output_list])
    # KNN_test_objectnet_shuffle(test_loader, model, args, [embed_list, label_list, output_list], path_visualization_results = path_visualization_results)
...
def LogisticRegression_test_objectnet_shuffle(val_loader, model, args, train_list):
    ...
    if os.path.exists(LR_path):
        ...
        }, False, filename='checkpoint.pth.tar', experiment_backup_folder='/proj/vondrick/augustine/')
        #}, False, filename='checkpointBL.pth.tar', experiment_backup_folder='/proj/vondrick/augustine/')
    ...
    while True:
        for i, example in enumerate(val_loader):
            ...
            prediction_score, pred = output.topk(maxk, 1, True, True)
            pred = pred.t()
            ...
            if args.visualize:
                ...
                class CombModel(nn.Module):
                    def __init__(self, model, LR):
                        ...
                        self.features1 = model

                        self.features2 = LR

                    def forward(self, x1):
                        o, fea, n = self.features1(x1)
                        x2 = self.features2(fea)
                        return x2
                new_model = CombModel(model, LR)
                ...
                print('Predicted:', path,  target[0].detach().cpu(),predicted_label, '(', prediction_score.squeeze()[0].item(), ')')
                top_pred_idx = pred_label_idx[0]

               
                gradient_shap = GradientShap(new_model)
                ...
                default_cmap = LinearSegmentedColormap.from_list('custom blue',
                                                                 [(0, '#ffffff'),
                                                                  (0.25, '#000000'),
                                                                  (1, '#000000')], N=256)
                truth =  idx_to_labels[str(target[0].detach().cpu().item())][1]
                temp = input.detach().cpu().numpy()
                # print("")
                temp = np.moveaxis(temp[0], 0, 2)
                mean = np.array([0.485, 0.456, 0.406])
                std = np.array([0.229, 0.224, 0.225])
                temp = np.clip((temp*std)+mean,0,1)
                transformed_img = temp   
                f = viz.visualize_image_attr_multiple(
                    ...
                    cmap=default_cmap,
                    show_colorbar=True)
                path_results_directory = "/proj/vondrick/www/amogh/examples/results/interpret/{}".format(truth)    
                f[0].suptitle('Truth: {} \n Predicted labels: {} \n Scores: {}'.format(truth, predicted_labels, predicted_scores), fontsize=12)
                save_directory = os.path.join(path_results_directory,"lr_output")
                if not os.path.exists(save_directory):
                    print("Makign directory at ", save_directory, path_results_directory)
                    os.makedirs(save_directory)
                path_save = os.path.join(save_directory,os.path.basename(path[0]))
            ...
            #    correct = pred.eq(target[i].cuda().view(1, -1).expand_as(pred))
            ...
            #print('{} Overlapping ImgNet'.format(args.dataset), int(i + 1), "/%d  top 1: %.5f, top 5: %.5f" %
