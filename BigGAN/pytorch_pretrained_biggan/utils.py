...
def truncated_noise_sample(batch_size=1, dim_z=128, truncation=1., seed=None):
    ...
    values = truncnorm.rvs(-2, 2, size=(batch_size, dim_z)).astype(np.float32)
...
def convert_img_to_model_output(img_list):
    img_arr = []
    for each in img_list:
        img_arr.append(np.expand_dims(np.asarray(np.float32(each), dtype=np.float32), axis=0))
    img_arr = np.concatenate(img_arr, axis=0)
    img_arr = img_arr/256*2.0-1
    img_arr = img_arr.transpose((0, 3, 1, 2))

    return img_arr

from PIL import Image
def load_images(filelist, basewidth=256, new_width=256, new_height=256):
    img_list = []
    for each in filelist:
        im = Image.open(each)
        print("load img size b4", im.size)
        im = im.convert("RGB")
        print("load img size", im.size)
...
def save_as_images(obj, file_name='output', j=0):
    ...
    for i, out in enumerate(img):
        current_file_name = file_name + '_%d_%d.png' % (i, j)
        logger.info("Saving image to {}".format(current_file_name))
        out.save(current_file_name, 'png')

def save_as_images_predictions(obj, pred, map_dict, file_name='output', j=0):
    """ Convert and save an output tensor from BigGAN in a list of saved images.
        Params:
            obj: tensor or numpy array of shape (batch_size, channels, height, width)
            file_name: path and beggingin of filename to save.
    ...
    for i, out in enumerate(img):
        num_pred = pred[i]
        print('num', num_pred)
        print("name", map_dict[num_pred[0]])

        current_file_name = file_name + '_%d_%d_%s.png' % (i, j, map_dict[num_pred[0]])
        logger.info("Saving image to {}".format(current_file_name))
        out.save(current_file_name, 'png')

def save_to_dataset(obj, pred, abandon_list, outputpath, map_dict, count, count_all):
    """ Convert and save an output tensor from BigGAN in a list of saved images.
        Params:
            obj: tensor or numpy array of shape (batch_size, channels, height, width)
            file_name: path and beggingin of filename to save.
    ...
    for i, out in enumerate(img):
        ...
        if abandon_list[i] == 0:
            num_pred = pred[i]
            # Discard if the same prediction on both neighbors, because we are looking for
            save_it=False
            if begin:
                begin = False
                if i< len(img)-1:
                ...
                if pred[i-1] != pred[i] or pred[i] != pred[i+1]:
                    save_it = True
            ...
            # current_file_name = file_name + '_%d_%d_%s.png' % (i, j, map_dict[num_pred[0]])
            if save_it:
                current_file_name = os.path.join(outputpath, "{}_{}_{}_{}.png".format(count, count_all,
                                                                                      num_pred[0], map_dict[num_pred[0]]))
                logger.info("Saving image to {}".format(current_file_name))
                out.save(current_file_name, 'png')
                count += 1
...
def save_allto_dataset(obj, pred, outputpath, map_dict, count, count_all):
    """ Convert and save an output tensor from BigGAN in a list of saved images.
        Params:
            obj: tensor or numpy array of shape (batch_size, channels, height, width)
            file_name: path and beggingin of filename to save.
    ...
    for i, out in enumerate(img):
        num_pred = pred[i]
        current_file_name = os.path.join(outputpath, "{}_{}_{}.png".format(count,
                                                                              num_pred[0], map_dict[num_pred[0]]))
        # logger.info("Saving image to {}".format(current_file_name))
        out.save(current_file_name, 'png')
        count += 1
