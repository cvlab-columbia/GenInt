...
def create_strip(inst, mode, layer, latents, x_comp, z_comp, act_stdev, lat_stdev, sigma, layer_start,
                 layer_end, num_frames=5, allrand=False):
    return _create_strip_impl(inst, mode, layer, latents, x_comp, z_comp, act_stdev,
                lat_stdev, None, None, sigma, layer_start, layer_end, num_frames, center=False, allrand=allrand)

# Strip where the sample is centered along the component before manipulation
def create_strip_centered(inst, mode, layer, latents, x_comp, z_comp, act_stdev, lat_stdev, act_mean, lat_mean, sigma,
                          layer_start, layer_end, num_frames=5, allrand=False, sigs=None, args=None):
    return _create_strip_impl(inst, mode, layer, latents, x_comp, z_comp, act_stdev,
                lat_stdev, act_mean, lat_mean, sigma, layer_start, layer_end, num_frames, center=True, allrand=allrand,
                              sigs=sigs, args=args)

def _create_strip_impl(inst, mode, layer, latents, x_comp, z_comp, act_stdev, lat_stdev, act_mean, lat_mean,
                       sigma, layer_start, layer_end, num_frames, center, allrand=False, sigs=None, args=None):
    ...
    if len(latents) > num_frames:
        ...
        return _create_strip_batch_sigma(inst, mode, layer, latents, x_comp, z_comp,
            act_stdev, lat_stdev, act_mean, lat_mean, sigma, layer_start, layer_end, num_frames, center, allrand,
                                         sigs=sigs, args=args)

# Batch over frames if there are more frames in strip than latents
import random
def _create_strip_batch_sigma(inst, mode, layer, latents, x_comp, z_comp, act_stdev, lat_stdev, act_mean, lat_mean,
                              sigma, layer_start, layer_end, num_frames, center, allrand, sigs=None, args=None):
    inst.close()
    ...
    if allrand:
        ...
        B = min(num_frames, 9) # Here, only allow 5 frames
    ...
    for i_batch in range(r):
        ...
        for i_lat in range(len(latents)):
            ...
            with torch.no_grad():
                ...
                if args and args.save_noise>0:
                    # print('lenz', len(z), 'z shape', z[0].shape)
                    for cc, img in enumerate(img_batch):
                        batch_frames[i_lat].append((img, z[0][cc].cpu().numpy()))
                else:
